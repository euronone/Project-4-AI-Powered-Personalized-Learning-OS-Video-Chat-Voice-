      ...
      <div className="grid gap-6 xl:grid-cols-[1.9fr_1fr]">
        <section className="space-y-4">
          <div className="surface-card p-5">
            <h1 className="text-xl font-bold text-slate-900 md:text-2xl">Understanding Newton's Laws of Motion</h1>
            <p className="mt-1 text-sm text-slate-500">Chapter 3 • Physics — Mechanics and Thermodynamics</p>
          </div>

          <div className="surface-card overflow-hidden">
            <div className="relative aspect-video bg-slate-900">
              <img src="/subjects/physics.jpg" alt="Video lesson" className="subject-image h-full w-full object-cover opacity-80" />
          ...
          <div className="surface-card p-4">
            <div className="flex flex-wrap items-center gap-2">
              <Link href="/chatbot" className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">
        ...
        <aside className="surface-card h-fit overflow-hidden">
          <div className="grid grid-cols-3 border-b border-slate-200 text-xs font-semibold">
            <button
              onClick={() => setActiveTab('recommendations')}
              className={activeTab === 'recommendations' ? 'border-b-2 border-slate-900 px-2 py-3 text-slate-900' : 'px-2 py-3 text-slate-500 hover:text-slate-700'}
            >
              Up Next
            </button>
            <button
              onClick={() => setActiveTab('transcript')}
              className={activeTab === 'transcript' ? 'border-b-2 border-slate-900 px-2 py-3 text-slate-900' : 'px-2 py-3 text-slate-500 hover:text-slate-700'}
            >
              Transcript
            </button>
            <button
              onClick={() => setActiveTab('notes')}
              className={activeTab === 'notes' ? 'border-b-2 border-slate-900 px-2 py-3 text-slate-900' : 'px-2 py-3 text-slate-500 hover:text-slate-700'}
            >
              Notes
            </button>
          </div>
            ...
            {activeTab === 'recommendations' ? (
              <div className="space-y-2">
                {upNextVideos.map((vid) => (
                  <Link key={vid.id} href="/video" className="card-hover flex gap-3 rounded-xl border border-slate-200 bg-white p-2">
                    <div className="relative h-[72px] w-[126px] overflow-hidden rounded-lg">
                      <img src={vid.coverImg} alt={vid.title} className="subject-image h-full w-full object-cover" />
                      <div className="subject-image-overlay absolute inset-0" />
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="line-clamp-2 text-xs font-semibold text-slate-900">{vid.title}</p>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={i} className={item.active ? 'rounded-xl border border-blue-200 bg-blue-50 p-3' : 'rounded-xl border border-slate-200 bg-white p-3'}>
                    <p className={item.active ? 'text-[11px] font-semibold text-blue-700' : 'text-[11px] font-semibold text-slate-500'}>{item.time}</p>
                    <p className={item.active ? 'mt-1 text-xs text-slate-800' : 'mt-1 text-xs text-slate-600'}>{item.text}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="space-y-2">
                <div className="rounded-xl border border-slate-200 bg-white p-2">
                  <textarea
                    ...
                    className="min-h-[220px] w-full resize-none rounded-lg border border-slate-200 p-3 text-sm text-slate-700 outline-none focus:border-slate-300"
                  />
                </div>
                <button className="w-full rounded-xl bg-slate-900 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">
                  Save Notes
                </button>
              </div>
            )}
